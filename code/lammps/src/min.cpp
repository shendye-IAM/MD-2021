...
Min::Min(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  // default values for min style genetic
  numIndividuals = 128;
  p = 0.1;
  localMinSteps = 100;
...
void Min::modify_params(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"dmax") == 0) {
      ...
      iarg += 2;
    } else if (strcmp(arg[iarg],"numIndividuals") == 0) {
        if (iarg+2 > narg) error->all(FLERR,"Illegal min_modify command");
        numIndividuals = utils::inumeric(FLERR,arg[iarg+1],false,lmp);
        iarg += 2;
    } else if (strcmp(arg[iarg],"p") == 0) {
        if (iarg+2 > narg) error->all(FLERR,"Illegal min_modify command");
        p = utils::numeric(FLERR,arg[iarg+1],false,lmp);
        iarg += 2;
    } else if (strcmp(arg[iarg],"localMinSteps") == 0) {
        if (iarg+2 > narg) error->all(FLERR,"Illegal min_modify command");
        localMinSteps = utils::inumeric(FLERR,arg[iarg+1],false,lmp);
        iarg += 2;
    } else {
